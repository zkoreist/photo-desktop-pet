...
| 测试 | Vitest 单测 **18 项**（移动/边界/拖拽/暂停/clamp/有界性/确定性 + DPI 150% + 多显示器负坐标 + 贴地锚点 anchorY），`npm test` |
...
| Phase D 裁剪/缩放/锚点 | 独立「编辑器」窗口（托盘「编辑图片…」打开）：裁剪框（拖拽+四角缩放）+ 缩放滑块 + 脚底锚点拖拽；非破坏保存（crop/scale/anchor 存 state，原图不动）；主窗口用同一套 `computeCropLayout` 渲染 + 窗口尺寸动态调整 |
